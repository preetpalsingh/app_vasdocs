...
$(document).on("click", ".show_Union_Login_modal", function () {
    ...
    $('#url_redirect').val($(this).data('url'));
});
...
$(document).on("submit",".sp_Union_Login_form",function(event) {
    ...
    var redirect_url        = $('#url_redirect').val();
    var email          = $('#email').val();
...
$( ".sp_device_search1" ).autocomplete({
    minLength: 3,
    ...
    $(document).on("change",".sp_device,.sp_device_search",function(event) {

        device = $('.sp_device').val();
        seacrh = $('.sp_device_search').val();

        if( device == '' && seacrh == '' ){

            $('.sp_seacrh').addClass('disabled');

        } else {

            $('.sp_seacrh').removeClass('disabled');
        }
    });

    $(document).on("click",".sp_seacrh",function(event) {
        console.log('clicked');
        device = $('.sp_device').val();
        seacrh = $('.sp_device_search').val();

        getContentWithSearch( seacrh, device );
    });
